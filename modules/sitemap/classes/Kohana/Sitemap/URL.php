...
class URL implements Sinterface {
	...
	{
		if ( ! \Kohana\Valid::max_length($location, 2048))
		{
			throw new \LengthException('The location was too long, maximum length of 2,048 characters.');
		}

		$location = \Kohana\Sitemap::encode($location);

		if ( ! \Kohana\Valid::url($location))
		{
	...
	{
		$this->attributes['lastmod'] = \Kohana\Sitemap::date_format($lastmod);
	 ...
	 * @param <type> $driver
	 */
	public function __construct(\Kohana\Sitemap\Sinterface $driver = NULL)
	{
		$this->driver = $driver;
	}
