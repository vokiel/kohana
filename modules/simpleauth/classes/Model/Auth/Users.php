...
class Users extends \Authmodeler\Core {
	...
	public function __construct($id = NULL)
	{
		...
		$auth_config =  \Kohana\Kohana::$config->load('simpleauth');
	...
	{
		$data =  \Kohana\DB::select('*')->from($this->table_name)->where($this->unique_field,'=',$unique)->and_where($this->password_field,'=',$pass)->execute();
	...
	public function user_exists($name = NULL, $second = NULL)
	{
		if (!empty($second))
		{
			return count(\Kohana\DB::select('id')->from($this->table_name)->where($this->unique_field, '=' , $name)->or_where($this->second_field, '=', $second)->execute());
		}
		else
		{
			return count(\Kohana\DB::select('id')->from($this->table_name)->where($this->unique_field, '=' ,$name)->execute());
		}
	}
