...
class Core {
	...
	public function __construct($config = 'simpleauth')
	{
		...
		$this->config = \Kohana\Kohana::$config->load($config);
		$this->session = \Kohana\Session::instance();
	}
	...
	public function logout($destroy = FALSE)
	{
		...
		if ($destroy === TRUE)
		{
			\Kohana\Session::instance()->destroy();
		}
		else
		{
		...
		\Kohana\Cookie::delete($this->config['cookie_key']);
	...
	{
		if ($token = \Kohana\Cookie::get($this->config['cookie_key']))
		{
			$token_model = new \Model\Auth\User\Tokens($token);
			...
			if ($token_model->user_agent === sha1(\Kohana\Request::user_agent('browser')))
			{
				...
				\Kohana\Cookie::set($this->config['cookie_key'], $token_model->token, strtotime($token_model->expires) - time());
