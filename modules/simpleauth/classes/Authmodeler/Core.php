...
class Core extends \Kohana\Model\Database
{
  ...
  public function save()
  {
    ...
    // Do an update
    if ($this->loaded()) {
      $result = count(\Kohana\DB::update($this->table_name)->set($data_to_save)->where($this->primary_key, '=', $this->data[$this->primary_key])->execute());
      if ($result) {
    ...
    {
      $id = \Kohana\DB::insert($this->table_name, array_keys($data_to_save))->values($data_to_save)->execute();
      if ($id) {
      ...
      if ($id AND !empty($this->hash_field)) {
        \Kohana\DB::update($this->table_name)->set(array($this->hash_field => sha1($this->table_name . $id . $this->hash_suffix)))->where($this->primary_key, '=', $this->data[$this->primary_key])->execute();
      }
  ...
  public function load($value, $key = NULL)
  {
    ...
    $data = \Kohana\DB::select($this->select)->from($this->table_name)->where($key, '=', $value)->execute();
  ...
  public function delete()
  {
    if (intval($this->data[$this->primary_key]) !== 0)
      return \Kohana\DB::delete($this->table_name)->where($this->primary_key, '=', $this->data[$this->primary_key])->execute();
  ...
  public function __wakeup()
  {
    ...
    $this->_db = \Kohana\Database::instance();
  }
