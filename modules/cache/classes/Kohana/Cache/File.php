 ...
 */
class File extends \Kohana\Cache implements GarbageCollect {
	...
	protected function __construct(array $config)
	{
		...
		try
		{
			$directory = \Kohana\Arr::get($this->_config, 'cache_dir', \Kohana\Kohana::$cache_dir);
		...
		{
			throw new \Kohana\Cache\Exception('Unable to create cache directory as a file already exists : :resource', array(':resource' => $this->_cache_dir->getRealPath()));
		}

		// Check the read status of the directory
		if ( ! $this->_cache_dir->isReadable())
		{
			throw new \Kohana\Cache\Exception('Unable to read from the cache directory :resource', array(':resource' => $this->_cache_dir->getRealPath()));
		}

		// Check the write status of the directory
		if ( ! $this->_cache_dir->isWritable())
		{
			throw new \Kohana\Cache\Exception('Unable to write to the cache directory :resource', array(':resource' => $this->_cache_dir->getRealPath()));
		}
	}
	...
	public function get($id, $default = NULL)
	{
		...
		try
		{
			...
			else
			{
				...
				if ($data->eof())
				{
					throw new \Kohana\Cache\Exception(__METHOD__.' corrupted cache file!');
				}
		...
		catch (\ErrorException $e)
		{
			...
			{
				throw new \Kohana\Cache\Exception(__METHOD__.' failed to unserialize cached object with message : '.$e->getMessage());
			}
	...
	public function set($id, $data, $lifetime = NULL)
	{
		...
		if ($lifetime === NULL)
		{
			...
			$lifetime = \Kohana\Arr::get($this->_config, 'default_expire', Cache::DEFAULT_EXPIRE);
		}
		...
		if ( ! $dir->isDir())
		{
			...
			{
				throw new \Kohana\Cache\Exception(__METHOD__.' unable to create directory : :directory', array(':directory' => $directory));
			}
		...
		catch (\ErrorException $e)
		{
			...
			if ($e->getCode() === E_NOTICE)
			{
				...
				throw new \Kohana\Cache\Exception(__METHOD__.' failed to serialize data for caching with message : '.$e->getMessage());
			}
		...
		try
		{
			...
			if ($file->isFile())
			{
				...
				catch (\ErrorException $e)
				{
					...
					{
						throw new \Kohana\Cache\Exception(__METHOD__.' failed to delete file : :file', array(':file' => $file->getRealPath()));
					}
				}
			}
			// Else, is directory
			elseif ($file->isDir())
			{
				...
				catch (\ErrorException $e)
				{
					...
					if ($e->getCode() === E_WARNING)
					{
						throw new \Kohana\Cache\Exception(__METHOD__.' failed to delete directory : :directory', array(':directory' => $file->getRealPath()));
					}
					throw $e;
				}
			}
	...
	protected function _make_directory($directory, $mode = 0777, $recursive = FALSE, $context = NULL)
	{
		...
		{
			throw new \Kohana\Cache\Exception('Failed to create the defined cache directory : :directory', array(':directory' => $directory));
		}
