...
class Kohana_Cache_Memcache extends Cache implements Cache_Arithmetic {
	...
	protected function __construct(array $config)
	{
		...
		{
			throw new \Kohana\Cache\Exception('Memcache PHP extention not loaded');
		}
		...
		$servers = \Kohana\Arr::get($this->_config, 'servers', NULL);

		if ( ! $servers)
		{
			// Throw an exception if no server found
			throw new \Kohana\Cache\Exception('No Memcache servers defined in configuration');
		}
		...
		foreach ($servers as $server)
		{
			...
			{
				throw new \Kohana\Cache\Exception('Memcache could not connect to host \':host\' using port \':port\'', array(':host' => $server['host'], ':port' => $server['port']));
			}
		}

		// Setup the flags
		$this->_flags = \Kohana\Arr::get($this->_config, 'compression', FALSE) ? MEMCACHE_COMPRESSED : FALSE;
	}
