...
class Cache {
	...
	public static function factory($cache, array $options = array())
	{
		...
		{
			$cache = \Kohana\Cache::instance($cache);
		}
	 ...
	 * @return  string
	 */
	public static function basic_cache_key_generator(\Kohana\Request $request)
	{
		$uri     = $request->uri();
		$query   = $request->query();
	...
	public function __construct(array $options = array())
	{
		...
		{
			$this->cache_key_callback('Cache::basic_cache_key_generator');
		}
	}
	 ...
	 * @return  [Response]
	 */
	public function execute(\Kohana\Request\Client $client, \Kohana\Request $request, \Kohana\Response $response)
	{
		if ( ! $this->_cache instanceof \Kohana\Cache)
		...
		if (in_array($request->method(), array(
			\Kohana\HTTP\Request::POST, 
			\Kohana\HTTP\Request::PUT, 
			\Kohana\HTTP\Request::DELETE)))
		{
			...
			$cache_control = \Kohana\HTTP\Header::create_cache_control(array(
				'no-cache',
				'must-revalidate'
			));
		...
		$response->headers(Cache::CACHE_STATUS_KEY, 
			Cache::CACHE_STATUS_MISS);
	 ...
	 * @return  void
	 */
	public function invalidate_cache(\Kohana\Request $request)
	{
		if (($cache = $this->cache()) instanceof \Kohana\Cache)
		{
	 ...
	 */
	public function cache(\Kohana\Cache $cache = NULL)
	{
		if ($cache === NULL)
			return $this->_cache;
	...
	public function cache_key_callback($callback = NULL)
	{
		...
		if ( ! is_callable($callback))
			throw new \Kohana\Exception('cache_key_callback must be callable!');
	 ...
	 * @return  string
	 */
	public function create_cache_key(\Kohana\Request $request, $callback = FALSE)
	{
		if (is_callable($callback))
			return call_user_func($callback, $request);
		else
			return Cache::basic_cache_key_generator($request);
	}
	 ...
	 * @return  boolean
	 */
	public function set_cache(\Kohana\Response $response)
	{
		$headers = $response->headers()->getArrayCopy();

		if ($cache_control = \Kohana\Arr::get($headers, 'cache-control'))
		{
			// Parse the cache control
			$cache_control = \Kohana\HTTP\Header::parse_cache_control($cache_control);
		...
		if ($expires = \Kohana\Arr::get($headers, 'expires') AND ! isset($cache_control['max-age']))
		{
	 ...
	 * @return  mixed
	 */
	public function cache_response($key, \Kohana\Request $request, \Kohana\Response $response = NULL)
	{
		if ( ! $this->_cache instanceof \Kohana\Cache)
			return FALSE;
		...
		if ($response === NULL)
		{
			...
			if ($this->_cache instanceof \Kohana\Cache\Arithmetic)
			{
				$hit_count = $this->_cache->increment(Cache::CACHE_HIT_KEY.$key);
			}
			else
			{
				$hit_count = $this->_cache->get(Cache::CACHE_HIT_KEY.$key);
				$this->_cache->set(Cache::CACHE_HIT_KEY.$key, ++$hit_count);
			}

			// Update the header to have correct HIT status and count
			$response->headers(Cache::CACHE_STATUS_KEY,
				Cache::CACHE_STATUS_HIT)
				->headers(Cache::CACHE_HIT_KEY, $hit_count);
		...
		else
		{
			...
			$response->headers(Cache::CACHE_STATUS_KEY,
				Cache::CACHE_STATUS_SAVED);

			// Set the hit count to zero
			$this->_cache->set(Cache::CACHE_HIT_KEY.$key, 0);
	 ...
	 */
	public function cache_lifetime(\Kohana\Response $response)
	{
