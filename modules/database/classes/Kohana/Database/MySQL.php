 ...
 */
class MySQL extends \Kohana\Database {
	...
	public function connect()
	{
		...
		catch (\Exception $e)
		{
			...
			throw new \Kohana\Exception(':error',
	...
	public function query($type, $sql, $as_object = FALSE, array $params = NULL)
	{
		...
		if (\Kohana\Kohana::$profiling)
		{
			...
			$benchmark = \Kohana\Profiler::start("Database ({$this->_instance})", $sql);
		}

		if ( ! empty($this->_config['connection']['persistent']) AND $this->_config['connection']['database'] !== MySQL::$_current_databases[$this->_connection_id])
		{
		...
		if (($result = mysql_query($sql, $this->_connection)) === FALSE)
		{
			if (isset($benchmark))
			{
				...
				\Kohana\Profiler::delete($benchmark);
			}

			throw new \Kohana\Exception(':error [ :query ]',
	...
	public function list_tables($like = NULL)
	{
		if (is_string($like))
		{
			...
			$result = $this->query(\Kohana\Database::SELECT, 'SHOW TABLES LIKE '.$this->quote($like), FALSE);
		}
		else
		{
			// Find all table names
			$result = $this->query(\Kohana\Database::SELECT, 'SHOW TABLES', FALSE);
		}
	...
	public function list_columns($table, $like = NULL, $add_prefix = TRUE)
	{
		...
		if (is_string($like))
		{
			...
			$result = $this->query(\Kohana\Database::SELECT, 'SHOW FULL COLUMNS FROM '.$table.' LIKE '.$this->quote($like), FALSE);
		}
		else
		{
			// Find all column names
			$result = $this->query(\Kohana\Database::SELECT, 'SHOW FULL COLUMNS FROM '.$table, FALSE);
		}
