...
class Query {
	...
	public function __toString()
	{
		try
		{
			...
			return $this->compile(\Kohana\Database::instance());
		}
		catch (\Exception $e)
		{
			return \Kohana\Exception::text($e);
		}
	}
	...
	public function cached($lifetime = NULL, $force = FALSE)
	{
		if ($lifetime === NULL)
		{
			...
			$lifetime = \Kohana\Kohana::$cache_life;
		}
	...
	public function execute($db = NULL, $as_object = NULL, $object_params = NULL)
	{
		...
		if ($this->_lifetime !== NULL AND $this->_type === \Kohana\Database::SELECT)
		{
			// Set the cache key based on the database instance name and SQL
			$cache_key = 'Database::query("'.$db.'", "'.$sql.'")';

			// Read the cache first to delete a possible hit with lifetime <= 0
			if (($result = \Kohana\Kohana::cache($cache_key, NULL, $this->_lifetime)) !== NULL
		...
		if (isset($cache_key) AND $this->_lifetime > 0)
		{
			...
			\Kohana\Kohana::cache($cache_key, $result->as_array(), $this->_lifetime);
		}
