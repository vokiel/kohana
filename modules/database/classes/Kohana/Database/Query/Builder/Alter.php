...
class Alter extends \Kohana\Database\Query\Builder {
	...
	public function __construct($table)
	{
		...
		parent::__construct(\Kohana\Database::ALTER, '');
	}
	 ...
	 */
	public function modify(\Kohana\Database\Column $column)
	{
		$this->_modify = $column;
	...
	{
		if ($object instanceof \Kohana\Database\Column)
		{
			$this->_add_columns[] = $object;
		}
		elseif ($object instanceof \Kohana\Database\Constraint)
		{
			$this->_add_constraints[] = $object;
		}
		else
		{
			throw new \Kohana\Exception('Unrecognised add object :obj', array(
				':obj' => $object
			));
		}
	 ...
	 * @return	string
	 */
	protected function _compile_add(\Kohana\Database $db)
	{
		$sql = '';
	 ...
	 * @return	string
	 */
	protected function _compile_modify(\Kohana\Database $db)
	{
	 ...
	 * @return	string
	 */
	protected function _compile_drop(\Kohana\Database $db)
	{
		if ( ! empty($this->_drop))
		{
			list($name, $type) = $this->_drop;
			return \Kohana\DB::drop($type, $name)->compile($db);
		}
