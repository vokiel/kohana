...
/**
 * The Kohana_HTTP\Header class provides an Object-Orientated interface
...
class Header extends \ArrayObject {
	...
	public static function accept_quality(array $parts)
	{
		...
		foreach ($parts_keys as $key)
		{
			...
			{
				$parsed[$value] = (float) HTTP\Header::DEFAULT_QUALITY;
			}
			else
			{
				$quality = $quality[2];
	...
	public static function parse_accept_header($accepts = NULL)
	{
		...
		if ($accepts === NULL)
			return array('*' => array('*' => (float) HTTP\Header::DEFAULT_QUALITY));

		// Parse the accept header qualities
		$accepts = HTTP\Header::accept_quality($accepts);
	...
	public static function parse_charset_header($charset = NULL)
	{
		if ($charset === NULL)
		{
			return array('*' => (float) HTTP\Header::DEFAULT_QUALITY);
		}

		return HTTP\Header::accept_quality(explode(',', (string) $charset));
	}
	...
	public static function parse_encoding_header($encoding = NULL)
	{
		...
		elseif ($encoding === '')
		{
			return array('identity' => (float) HTTP\Header::DEFAULT_QUALITY);
		}
		else
		{
			return HTTP\Header::accept_quality(explode(',', (string) $encoding));
		}
	}
	...
	public static function parse_language_header($language = NULL)
	{
		...
		$language = HTTP\Header::accept_quality(explode(',', (string) $language));
	...
	public function accepts_at_quality($type, $explicit = FALSE)
	{
		...
		if ($this->_accept_content === NULL)
		{
			...
			$this->_accept_content = HTTP\Header::parse_accept_header($accept);
		}
	...
	public function accepts_charset_at_quality($charset)
	{
		if ($this->_accept_charset === NULL)
		{
			if ($this->offsetExists('Accept-Charset'))
			{
				...
				$this->_accept_charset = HTTP\Header::parse_charset_header($charset_header);
			}
			else
			{
				$this->_accept_charset = HTTP\Header::parse_charset_header(NULL);
			}
		}
	...
	public function accepts_encoding_at_quality($encoding, $explicit = FALSE)
	{
		if ($this->_accept_encoding === NULL)
		{
			...
			$this->_accept_encoding = HTTP\Header::parse_encoding_header($encoding_header);
		}
		...
		if ($explicit === FALSE)
		{
			...
			{
				return (float) HTTP\Header::DEFAULT_QUALITY;
			}
		}
	...
	public function accepts_language_at_quality($language, $explicit = FALSE)
	{
		if ($this->_accept_language === NULL)
		{
			...
			$this->_accept_language = HTTP\Header::parse_language_header($language_header);
		}
	 ...
	 * @since   3.2.0
	 */
	public function send_headers(\HTTP\Response $response = NULL, $replace = FALSE, $callback = NULL)
	{
		if ($response === NULL)
		{
