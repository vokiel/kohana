...
class Request implements \HTTP\Request {
	 ...
	 * @uses    Route::matches
	 */
	public static function factory($uri = TRUE, HTTP\Cache $cache = NULL, $injected_routes = array())
	{
	 ...
	 * @uses    Route::matches
	 */
	public function __construct($uri, HTTP\Cache $cache = NULL, $injected_routes = array())
	{
		...
		if (Request::$initial === NULL OR strpos($uri, '://') === FALSE)
		{
			...
			// Apply the client
			$this->_client = new \Request\Client\Internal(array('cache' => $cache));
		}
		else
		{
			// Create a route
			...
			// Setup the client
			$this->_client = Request\Client\External::factory(array('cache' => $cache));
		}
	}
