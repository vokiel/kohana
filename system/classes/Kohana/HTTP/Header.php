...
class Header extends \ArrayObject {
	...
	public function accepts_at_quality($type, $explicit = FALSE)
	{
		...
		{
			$mime = \Kohana\Kohana::$config->load('mimes.'.$type);
	 ...
	 * @since   3.2.0
	 */
	public function send_headers(\Kohana\Response $response = NULL, $replace = FALSE, $callback = NULL)
	{
		if ($response === NULL)
		{
			// Default to the initial request message
			$response = \Kohana\Request::initial()->response();
		}
		...
		$processed_headers = array($protocol.' '.$status.' '.\Kohana\Response::$messages[$status]);
		...
		foreach ($headers as $header => $value)
		{
			...
			$processed_headers[] = \Kohana\Text::ucfirst($header).': '.$value;
		}

		if ( ! isset($headers['content-type']))
		{
			$processed_headers[] = 'Content-Type: '.\Kohana\Kohana::$content_type.'; charset='.\Kohana\Kohana::$charset;
		}

		if (\Kohana\Kohana::$expose AND ! isset($headers['x-powered-by']))
		{
			$processed_headers[] = 'X-Powered-By: '.\Kohana\Kohana::version();
		}
	...
	protected function _send_headers_to_php(array $headers, $replace)
	{
		...
		foreach ($headers as $key => $line)
		{
			if ($key == 'Set-Cookie' AND is_array($line))
			{
				...
				{
					\Kohana\Cookie::set($name, $value['value'], $value['expiration']);
				}
