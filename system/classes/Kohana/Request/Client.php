...
abstract class Client {
	 ...
	 * @uses    [Profiler]
	 */
	public function execute(\Kohana\Request $request)
	{
		...
		// Execute the request
		$orig_response = $response = \Kohana\Response::factory();

		if (($cache = $this->cache()) instanceof \Kohana\HTTP\Cache)
		...
		foreach ($this->header_callbacks() as $header => $callback)
		{
			if ($response->headers($header))
			{
				...
				if ($cb_result instanceof \Kohana\Request)
				{
				...
				elseif ($cb_result instanceof \Kohana\Response)
				{
	 ...
	 * @since   3.2.0
	 */
	abstract public function execute_request(\Kohana\Request $request, \Kohana\Response $response);
	 ...
	 */
	public function cache(\Kohana\HTTP\Cache $cache = NULL)
	{
		if ($cache === NULL)
			return $this->_cache;
	 ...
	 * @param Request_Client $client
	 */
	public static function on_header_location(\Kohana\Request $request, \Kohana\Response $response, Client $client)
	{
		...
		if ($client->follow() AND in_array($response->status(), array(201, 301, 302, 303, 307)))
		{
			...
			$follow_request = \Kohana\Request::factory($response->headers('Location'))
			                         ->method($follow_method)
			                         ->headers(\Kohana\Arr::extract($request->headers(), $client->follow_headers()));

			if ($follow_method !== \Kohana\Request::GET)
			{
